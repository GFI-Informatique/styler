...
Styler.SpatialFilterPanel = Ext.extend(Styler.BaseFilterPanel, {
    ...
    createIsochrone: function(params) {
        var wkt = new OpenLayers.Format.WKT(),
            json = new OpenLayers.Format.JSON();
        OpenLayers.Request.POST({
            url: this.isochroneService + params,
            data: wkt.write(this.feature),
            success: function(response) {
                var bWkt = json.read(response.responseText)['geometry'],
                    bGeometry = wkt.read(bWkt).geometry,
    ...
    isoWindow: function() {
        ...
        if (Ext.getCmp("iso_window")) {
            ...
            var mandatoryItems = [{
                ...
                border: false,
                items: [{
                    html: '<input id="iso_hour" type="number" step=5 style="border:solid 1px rgba(0,126,195,0.6); width: 60px; border-radius:7px text-align:center;" />',
                    id: "inputhour",
                    border: false
                }, {
                    xtype: 'displayfield',
                    ...
                    html: '<input id="iso_min" type="number" step=5 style="border:solid 1px rgba(0,126,195,0.6); width: 60px; border-radius:7px text-align:center;" />',
                    id: "inputmin",
                    border: false,
                }, {
                    xtype: 'displayfield',
    ...
    createFilterItems: function() {
        ...
        if (this.bufferService && OpenLayers.Format &&
        	...
        	// controler que geometry = point pour afficher le bouton. Pas de centroïde.
            buttonPanels.push({
                items: [{
                    xtype: "button",
